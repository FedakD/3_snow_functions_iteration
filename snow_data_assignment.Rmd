...
fd_monthly_temp <- forcing_data %>%
  ...
  summarize(mean_temp=mean(air.temp..K.))
years <- list(seq(2005,2010,by=1))
#ggplot(fd_monthly_temp, aes(x=month, y=mean_temp, color=site)) +
#  geom_line()

for(year in years){
  print(ggplot(fd_monthly_temp, aes(x=month, y=mean_temp, color=site, year=years)) +  
    geom_line() + 
